...
###   trustcoincore.org and trustcoin.org and compares them.
...
WORKINGDIR="/tmp/trustcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://trustcoincore.org"
HOST2="https://trustcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="trustcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'trustcoin-',
...
if [ $? -ne 0 ]; then
   echo "trustcoin.org failed to provide signature file, but trustcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "trustcoin.org and trustcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Trustcoin Core binary release signing key installed?"
   fi
